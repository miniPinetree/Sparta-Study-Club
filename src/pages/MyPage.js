          ...
          <ContentBox>
            <ItemBox>
              ...
              <QuestBox className="questlist">
                <Text bold>
                  오늘의 퀘스트! 현재 달성률:{" "}
                  <Point>{dayRate ? dayRate : 0}%</Point>
                </Text>
                <TodoInput
                  placeholder={
                    user?.studySetTime
                      ? `${user?.nickname}님, 오늘의 목표를 정해주세요:)`
                      : "목표 시간을 선택해주세요 !"
                  }
                  onKeyPress={(e) => {
                    if (e.key === "Enter") {
                      if (!user.studySetTime) {
                        Swal.fire({
