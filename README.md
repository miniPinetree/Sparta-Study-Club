...
- 로그인 정보 유지 시 보안을 위해 유저정보를 클라이언트 웹 저장소에 저장해두고 싶지 않았고 매번 서버에서 토큰 유효성 검증을 해주기를 제안하였다. 
그러나 클라이언트에서 보관하는 정보만을 가지고 로그인 유지 처리를 진행하더라도 거의 동시에 토큰 유효성 검증이 이루어진다는 것을 알고 로그인 유지 방식을 타협할 수 있었다.
자세한 과정은 이러하다. 로그인 체크 뿐 아니라 모든 API 요청 시 Header에 access token이 담겨 전송되고 있었으므로 회원 정보가 담긴 페이지가 렌더링 되는 과정에서 호출되는
**GET API를 통해 매번 토큰 유효성 검증, 즉 사용자 권한 확인이 이루어지므로 보안 문제를 보완할 수 있다고 생각되어졌다.**
