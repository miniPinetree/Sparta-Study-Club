...
- React-Node.js에서 주로 사용하는 socket.io대신 ws프로토콜을 이용하여 웹소켓을 활용한 실시간 채팅을 구현하였다.
...
- 프로그레스 바는 시간 경과율을 너무 자주 계산할 시에 성능 저하를 일으킬 수 있고 변화하는 정도도 미미하므로 프로그레스 바가 10%씩 이동하도록 사용자가 설정한 시간의 10분의 1의 시간이 흐를 때마다 경과율을 계산하고 프로그레스 바를 변동시키도록 설정하였다. 
